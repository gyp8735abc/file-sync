...
public class FileDeleteEvent implements FileEvent {
	private GroupInfo group;
	private File file;
	private FileChangeType eventType = FileChangeType.DELETE;
	private Date recordTime;
	private int state = 0;

	public FileDeleteEvent(GroupInfo group, File file) {
		this.group = group;
		this.file = file;
		recordTime = new Date();
	}
	...
	public boolean equals(Object obj) {
		if (obj instanceof FileDeleteEvent) {
			FileDeleteEvent t = (FileDeleteEvent) obj;
	...
	public void execute() {
		...
		fileInfo = FileManagerDao.getFileInfo(group, fileGroupPath);
		if(fileInfo != null){
		...
		for (int i = 0; i < members.size(); i++) {
			...
			String url = String.format("http://%s:%d/rsync/fileDeleted", member.getIp(), member.getPort());
	...
	public String toString() {
		return String.format("gid=%s,file=%s,event=%s", group.getGroupNo(), file.getAbsolutePath(), "Delete");
	}
	@Override
